...
metadata validators = [
    
    {
      ...
      configuration: {
          bindToTrait: uk.co.motorway#handler
          selector: """
              :not([@trait|uk.co.motorway#handler: @{language} = typescript, java, python])
          """
          messageTemplate: """
              @{trait|uk.co.motorway#handler|language} is not supported by type-safe-api.
    ...
    
    {
      ...
      configuration: {
          bindToTrait: uk.co.motorway#handler
          selector: """
              [@trait|uk.co.motorway#handler: @{language} = typescript, java, python]
              :not([@trait|uk.co.motorway#handler: @{language} = typescript])
          """
          messageTemplate: """
              @@handler language @{trait|uk.co.motorway#handler|language} cannot be referenced unless a handler project is configured for this language.
